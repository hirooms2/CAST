...
class Trainer:
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            model.train()
            epoch_loss = 0
            for (user_category, user_subCategory, user_title_text, user_title_mask, user_body_text, user_body_mask, user_news_mask, user_history_mask, user_history_position,\
                news_category, news_subCategory, news_title_text, news_title_mask, news_body_text, news_body_mask, news_mask) in tqdm(train_dataloader):
                ...
                user_news_mask = user_news_mask.cuda(non_blocking=True)                                                                                                         # [batch_size, max_history_num, max_title_length, max_title_length + max_body_length]
                ...
                news_body_text = news_body_text.cuda(non_blocking=True)                                                                                                         # [batch_size, 1 + negative_sample_num, max_body_length]
                news_body_mask = news_body_mask.cuda(non_blocking=True)                                                                                                         # [batch_size, 1 + negative_sample_num, max_body_length]
                news_mask = news_mask.cuda(non_blocking=True)                                                                                                                   # [batch_size, 1 + negative_sample_num, max_title_length, max_title_length + max_body_length]

                logits = model(user_category, user_subCategory, user_title_text, user_title_mask, user_body_text, user_body_mask, user_news_mask, user_history_mask, user_history_position,\
                                news_category, news_subCategory, news_title_text, news_title_mask, news_body_text, news_body_mask, news_mask) # [batch_size, 1 + negative_sample_num]
                ...
                epoch_loss += float(loss) * user_category.size(0)
            ...
            if self.dataset == 'MIND_small':
                ...
                print('AUC = {:.4f}\tMRR = {:.4f}\tnDCG@5 = {:.4f}\tnDCG@10 = {:.4f}'.format(auc, mrr, ndcg, ndcg10))
